...
public class ExampleModelBrowserServlet extends DataModelBrowserServlet {
	...
	protected String generatePersistTestData(EPackage ePackage) {
		try {
			final List<EPackage> ePackages = Collections
					.singletonList(ePackage);
			...
			modelDataGenerator.setMaxObjects(10000);
			...
			for (EObject eObject : modelDataGenerator.getAllEObjects()) {
				if (countByEClass.containsKey(eObject.eClass())) {
					countByEClass.put(eObject.eClass(),
							countByEClass.get(eObject.eClass()) + 1);
				} else {
			...
			for (EClass eClass : countByEClass.keySet()) {
				sb.append(eClass.getName()
						+ ": " + countByEClass.get(eClass) + "<br/>"); //$NON-NLS-1$//$NON-NLS-2$
			}

			sb.append("</body></html>"); //$NON-NLS-1$

			final List<Object> modelObjects = emfModelConverter
					.convert(modelDataGenerator.getResult());
